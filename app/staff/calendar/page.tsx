...
function CalendarView() {
  ...
  const downloadReport = async () => {
    ...
    try {
      ...
      link.href = url;
      link.download = `credarc-meetings-${range.from}-to-${range.to}.csv`;
            ...
            >
              <div className="sticky top-0 flex items-center justify-between border-b border-navy-500/10 dark:border-white/10 bg-white dark:bg-surface-dark-card px-6 py-5">
                <p className="font-serif text-lg text-navy-500 dark:text-white">
                  {dayFmt.format(day)}
                  {isToday ? (
                    <span className="ml-2 rounded-full bg-[#0575DD]/25 px-2 py-0.5 align-middle text-[11px] font-semibold uppercase tracking-[0.14em] text-gold-500">
                      Today
                    </span>
                  ) : null}
                </p>
