...
/** CREDARC's actual office address (from the main website's footer) — the
...
export async function sendMeetingEmail(options: {
  ...
  const where = location || CREDARC_OFFICE_ADDRESS;
  ...
  const withLine = clientEmail
    ? [staffName, "Credarc Technologies Pvt. Ltd."].filter(Boolean).join(", ")
          ...
          <table role="presentation" width="480" cellpadding="0" cellspacing="0" style="background-color:#ffffff;border-radius:12px;overflow:hidden;font-family:Arial,Helvetica,sans-serif;">
            ${
              ...
              <td align="right" style="background-color:#022436;padding:18px 32px;">
                <img src="cid:${LOGO_CID}" alt="CREDARC" height="30" style="display:inline-block;height:30px;width:auto;border:0;" />
              </td>
            </tr>`
                : ""
            }
            ...
            <tr>
              <td style="padding:20px 32px 28px 32px;">
                ...
                <p style="margin:0 0 16px 0;font-size:14px;color:#111827;">Regards,<br />Credarc Technologies Pvt. Ltd.</p>
