...
/**
 * Sends mail from info@credarc.in, over the same GoDaddy Workspace Email SMTP
 * server the main CREDARC website's contact form already uses.
 */
...
export async function sendMail(options: {
  ...
  const { replyTo = "no-reply@credarc.in", ...rest } = options;

  await getTransporter().sendMail({
    from: `"CREDARC" <${SMTP_USER}>`,
    replyTo,
    ...rest,
  });
}
