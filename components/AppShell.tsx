...
function ThemedShell({ children }: { children: ReactNode }) {
  ...
  return (
    <div
      className={`flex min-h-screen flex-col transition-colors duration-200 ${theme === "dark" ? "dark bg-surface-dark" : "bg-stone-50"
        }`}
    >
      {children}
    </div>
...
function ShellChrome({ children }: { children: ReactNode }) {
  ...
  const waitingLabel = `${pending.length} ${pending.length === 1 ? "visitor" : "visitors"
    } waiting at the door`;
      ...
      <header className="sticky top-0 z-30 border-b border-navy-500/10 bg-white/90 backdrop-blur-xl dark:border-white/10 dark:bg-surface-dark/90">
        <div className="mx-auto flex max-w-7xl items-center justify-between gap-4 px-4 py-3 sm:px-6">
          <Link href={isReception ? "/reception" : "/staff"} className="flex shrink-0 items-center gap-3">
            <Image src={theme == 'dark' ? lightLogo : logo} alt="CREDARC" width={192} height={57} className="h-14 w-auto object-contain" />
          </Link>
          ...
          <nav aria-label="Main" className="hidden items-center gap-1 sm:flex">
            {nav.map((item) => (
              <Link
                ...
                className={`flex min-h-10 items-center rounded-lg px-4 text-sm font-medium transition-colors duration-200 ${pathname === item.href
