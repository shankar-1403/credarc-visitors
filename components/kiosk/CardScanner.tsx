      ...
      <div className="mt-5 flex flex-1 flex-col items-center justify-center rounded-2xl border border-white/10 bg-black/30">
        {state === "live" ? (
          <div className="relative w-full overflow-hidden rounded-2xl">
            ...
            <div
              aria-hidden
              className="pointer-events-none absolute inset-6 rounded-2xl border-2 border-[#0575DD]/70"
            />
          </div>
        ) : state === "reading" ? (
          <div className="flex flex-col items-center gap-4 px-6 py-10 text-center">
            <span
              aria-hidden
              className="size-10 animate-spin rounded-full border-2 border-[#0575DD]/30 border-t-[#0575DD]"
            />
            <p className="text-sm text-white/60">Reading your card&hellip;</p>
          </div>
        ) : (
          <div className="flex flex-col items-center gap-4 px-6 py-10 text-center">
            <span className="flex size-16 items-center justify-center rounded-full bg-[#0575DD]/15">
              {scannedOnce ? (
                <IconCheck className="size-7 text-[#0575DD]" />
              ) : (
                <IconCamera className="size-7 text-[#0575DD]" />
              )}
            </span>
        ...
        {state === "live" ? (
          <div className="flex gap-3">
            ...
            <button
              type="button"
              onClick={capture}
              className="flex min-h-14 flex-1 cursor-pointer items-center justify-center gap-2 rounded-2xl bg-[#0575DD] text-base font-semibold text-brand-deep transition-all duration-200 hover:bg-[#09B6F1]"
            >
          ...
          <button
            ...
            className="flex min-h-14 w-full cursor-pointer items-center justify-center gap-2 rounded-2xl bg-[#0575DD] text-base font-semibold text-brand-deep transition-all duration-200 hover:bg-[#09B6F1] disabled:cursor-not-allowed disabled:opacity-60"
          >
            <IconCamera className="size-5" />
            {state === "starting"
              ? "Opening camera…"
